...
using namespace my_ptmalloc::jemalloc;
...
static void test_various_sizes() {
    for (size_t sz : {1, 8, 16, 24, 32, 64, 128, 256, 512, 1024, 2048,
                       4096, 8192, 14336, 16384, 65536, 131072, 524288,
...
static void test_usable_size() {
    for (size_t sz = 1; sz <= 16384; sz <<= 1) {
        void* p = je_malloc(sz);
        CHECK(p != nullptr);
        CHECK(je_usable_size(p) >= sz);
        je_free(p);
    }
    PASS("usable_size");
}
...
static void test_random_fuzz() {
    ...
    size_t ss[S] = {};
    std::mt19937 rng(42);
    std::uniform_int_distribution<size_t> sd(1, 16384);
    ...
    for (int i = 0; i < 5000; i++) {
        ...
        if (a == 0) {
            if (p[idx]) je_free(p[idx]);
            ss[idx] = sd(rng);
            p[idx] = je_malloc(ss[idx]);
            CHECK(p[idx] != nullptr);
            std::memset(p[idx], idx & 0xFF, ss[idx] < 256 ? ss[idx] : 256);
        } else if (a == 1) { je_free(p[idx]); p[idx] = nullptr; }
        else { p[idx] = je_realloc(p[idx], sd(rng)); CHECK(p[idx] != nullptr); }
    }
    for (int i = 0; i < S; i++) if (p[i]) je_free(p[i]);
    PASS("random_fuzz");
}
...
static void test_multithreaded() {
    ...
    for (int t = 0; t < N; t++) {
        th.emplace_back([t, &err]() {
            ...
            std::uniform_int_distribution<size_t> sd(1, 4096);
            constexpr int S = 32;
            void* p[S] = {};
            for (int i = 0; i < O; i++) {
                int idx = std::uniform_int_distribution<int>(0, S-1)(rng);
                if (p[idx]) { je_free(p[idx]); p[idx] = nullptr; }
                p[idx] = je_malloc(sd(rng));
                if (!p[idx]) err++;
